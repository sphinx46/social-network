...
public class HomeController {
     ...
     * @return ответ с информацией о статусе аутентификации
     */
    ...
    public Mono<ResponseEntity<String>> home(
        ...
        if (oidcUser != null) {
            log.info("ШЛЮЗ_ГЛАВНАЯ_АУТЕНТИФИЦИРОВАН: "
                    + "пользователь {} запросил главную страницу",
                    oidcUser.getPreferredUsername());
            return Mono.just(ResponseEntity.ok()
                    .body("Authentication successful! Welcome, "
                            + oidcUser.getPreferredUsername()
            ...
            log.debug("ШЛЮЗ_ГЛАВНАЯ_НЕАУТЕНТИФИЦИРОВАН: "
                    + "неаутентифицированный запрос главной страницы");
            return Mono.just(ResponseEntity.ok()
                    .body("API Gateway is running. "
                            + "Please authenticate at:\n"
                            + "http://localhost:8082"
                            + "/oauth2/authorization/keycloak\n\n"
                            + "To force login screen (disable SSO): "
                            + "/oauth2/authorization/keycloak?prompt=login"));
        }
    }
}
