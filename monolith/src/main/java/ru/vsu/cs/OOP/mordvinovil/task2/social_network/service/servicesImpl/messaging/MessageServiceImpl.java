...
public class MessageServiceImpl implements MessageService {
    ...
    public PageResponse<MessageResponse> getConversation(Long otherUserId, User currentUser, PageRequest pageRequest) {
        ...
        centralLogger.logInfo("ПЕРЕПИСКА_ПОЛУЧЕНИЕ",
                "Получение переписки", context);
        ...
        try {
            ...
            centralLogger.logInfo("ПЕРЕПИСКА_ПОЛУЧЕНА",
                    "Переписка успешно получена", resultContext);

            return PageResponse.of(messages.map(
                    message -> entityMapper.map(message, MessageResponse.class))
            );
        } catch (Exception e) {
            centralLogger.logError("ПЕРЕПИСКА_ОШИБКА_ПОЛУЧЕНИЯ",
