...
public final class UploadProducer {
    ...
    private final KafkaTemplate<String, MessageImageUploadedEvent> messageImageUploadedEventKafkaTemplate;
    ...
    /**
     * Отправляет событие загрузки изображения сообщения в Kafka.
     * Преобразует сущность MediaEntity в событие MessageImageUploadedEvent и отправляет в топик.
     * Использует ownerId в качестве ключа для гарантии порядка сообщений одного пользователя.
     * Обрабатывает успешную отправку и ошибки через CompletableFuture.
     *
     * @param entity сущность медиа-файла, содержащая информацию о загруженном изображении сообщения
     * @param messageId идентификатор сообщения
     * @throws RuntimeException если произошла критическая ошибка при подготовке или отправке сообщения
     */
    public void sendMessageImageUploadedEvent(final MediaEntity entity, final UUID messageId) {
        try {
            log.info("MESSAGE_IMAGE_EVENT_ОТПРАВКА_НАЧАЛО: mediaId={} ownerId={}",
                    entity.getId(), entity.getOwnerId());

            final MessageImageUploadedEvent event = mapper.map(entity, MessageImageUploadedEvent.class);
            event.setMessageId(messageId);
            final String key = event.getOwnerId().toString();

            final CompletableFuture<SendResult<String, MessageImageUploadedEvent>> future =
                    messageImageUploadedEventKafkaTemplate.send(MESSAGE_IMAGE_TOPIC, key, event);

            future.thenAccept(result -> {
                log.info("MESSAGE_IMAGE_EVENT_ОТПРАВКА_УСПЕХ: mediaId={} ownerId={} partition={} offset={}",
                        entity.getId(), key,
                        result.getRecordMetadata().partition(),
                        result.getRecordMetadata().offset());
            }).exceptionally(failure -> {
                log.error("MESSAGE_IMAGE_EVENT_ОТПРАВКА_ОШИБКА: mediaId={} ownerId={}",
                        entity.getId(), key, failure);
                return null;
            });

        } catch (Exception e) {
            log.error("MESSAGE_IMAGE_EVENT_ОТПРАВКА_КРИТИЧЕСКАЯ_ОШИБКА: mediaId={} ownerId={}",
                    entity.getId(), entity.getOwnerId(), e);
            throw new RuntimeException("Ошибка при отправке события в Kafka", e);
        }
    }
